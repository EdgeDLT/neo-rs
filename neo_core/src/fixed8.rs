...
const TOTAL_FIXED8_HEX = new BN(2).pow(64);
  ...
  pub const MAX_VALUE = new Fixed8(MAX_FIXED8_HEX.div(DECIMALS));

  // The minimum Fixed8 is obtained by dividing 0x8000000000000000 (= -9223372036854775808) with 10^8.
  pub const MIN_VALUE = new Fixed8(MIN_FIXED8_HEX.div(DECIMALS));
...
export class Fixed8 extends BN {
  ...
  pub static fromHex(hex: string): Fixed8 {
    if (hex.length > 16) {
      throw new Error(
  ...
  pub static fromReverseHex(hex: string): Fixed8 {
    return this.fromHex(reverseHex(hex));
  }

  pub constructor(input?: number | string | BN, base = 10) {
    if (!input) {
      input = "0";
    }
   ...
   */
  pub equals(other: string | number | Fixed8 | BN): boolean {
   ...
   * @return {Fixed8}
   */
  pub round(dp = 0, rm?: BN.RoundingMode): Fixed8 {
   ...
   * @alias div
   */
  pub dividedBy(n: string | number | BN, base?: number): Fixed8 {
    return new Fixed8(super.dividedBy(n, base));
  }

  pub div(n: string | number | BN, base?: number): Fixed8 {
   ...
   * @alias mul
   */
  pub times(n: string | number | BN, base?: number): Fixed8 {
    return new Fixed8(super.times(n, base));
  }

  pub mul(n: string | number | BN, base?: number): Fixed8 {
   ...
   * @alias add
   */
  pub plus(n: string | number | BN, base?: number): Fixed8 {
    return new Fixed8(super.plus(n, base));
  }

  pub add(n: string | number | BN, base?: number): Fixed8 {
   ...
   * @alias sub
   */
  pub minus(n: string | number | BN, base?: number): Fixed8 {
    return new Fixed8(super.minus(n, base));
  }

  pub sub(n: string | number | BN, base?: number): Fixed8 {
